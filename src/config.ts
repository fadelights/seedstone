/**
 * Central tuning schema — every visual and performance knob for the renderer
 ...
 * knob samples an independent hash of its dot-path and the seed, so adding,
 * removing, or pinning one never affects the others. The config lab
 * auto-discovers all knobs from this schema.
 *
...
function c(value: number, min: number, max: number, step: number): ConfigKnob {
  return { mode: 'config', value, min, max, step };
}

/** Seed-generated: sampled uniformly from [min, max] for each gem. */
function d(min: number, max: number): ConfigKnob {
...
function pick(options: () => string[], pinned?: string): PickKnob {
 ...
 * Fields are still wrapped in `ConfigKnob` / `PickKnob` objects that carry
 * metadata (min, max, step, mode). Use this type when you need to *inspect*
 * the schema: walking its tree, reading ranges, building a knobs UI, etc.
 */
...
type Resolved<T> =
  T extends ConfigKnob ? number
  : T extends PickKnob ? string
...
/** Override a scalar knob.
 *  - plain number          → pin to 'config' with that value
 *  - `{ mode: 'config' }` → pin to 'config' (value optional; defaults to knob's current value)
 *  - `{ mode: 'dna' }`    → make seed-generated (even if the schema default was c()) */
export type KnobOverride = number | { mode: 'config'; value?: number } | { mode: 'dna' };

/** Override a categorical pick knob.
 *  - plain string          → pin to 'config' with that choice
 *  - `{ mode: 'config' }` → pin to 'config' (value optional)
 *  - `{ mode: 'dna' }`    → make seed-picked */
export type PickOverride = string | { mode: 'config'; value?: string } | { mode: 'dna' };

type SchemaOverride<T> =
  T extends ConfigKnob ? KnobOverride
  : T extends PickKnob ? PickOverride
  : T extends readonly unknown[] ? T
 ...
 * and each numeric knob also accepts `{ mode: 'dna' | 'config', value? }` to flip
 * whether the value is seed-generated or fixed — overriding the schema default.
 */
export type SeedstoneConfigOverrides = SchemaOverride<SeedstoneSchema>;

// ── Schema resolution ─────────────────────────────────────────────────────────

export function isKnob(v: unknown): v is ConfigKnob {
  return typeof v === 'object' && v !== null && 'mode' in v && 'min' in v;
}

export function isPick(v: unknown): v is PickKnob {
...
/**
 * Apply numeric overrides onto the schema tree. Overriding a knob pins it:
 * the value is fixed and the mode becomes 'config', even for d() knobs —
 * so `{ gem: { hue: 200 } }` makes every gem render with that hue.
 */
  ...
  function merge<T extends object>(base: T, patch: Record<string, unknown>): T {
    ...
    for (const [key, value] of Object.entries(patch)) {
      ...
      if (isKnob(baseValue)) {
        ...
        } else if (isPlainObject(value) && 'mode' in value) {
          const ov = value as { mode: 'config' | 'dna'; value?: number };
          merged[key] = { ...baseValue, mode: ov.mode, ...(ov.value !== undefined && { value: ov.value }) };
        }
      } else if (isPick(baseValue)) {
        if (typeof value === 'string') {
          merged[key] = { ...baseValue, mode: 'config', value };
        } else if (isPlainObject(value) && 'mode' in value) {
          const ov = value as { mode: 'config' | 'dna'; value?: string };
          merged[key] = { ...baseValue, mode: ov.mode, ...(ov.value !== undefined && { value: ov.value }) };
        }
...
/**
 * Resolve a schema tree to plain values. Fixed knobs collapse to their value;
 * generated ones are sampled using the seed and the knob's dot-path as the
 * hash label. Without a seed, generated knobs resolve to their range midpoint
