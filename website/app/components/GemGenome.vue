...
const gemColor = computed(
  () => `oklch(0.62 ${(0.16 + gnSat.value * 0.1).toFixed(3)} ${gnHue.value.toFixed(1)})`,
);
  ...
  return [
    {
      label: "Saturation",
      pct: g
        ? statPct(gnSat.value, g.saturation.min, g.saturation.max)
        : Math.round(gnSat.value * 100),
      value: `${Math.round(gnSat.value * 100)}%`,
    },
    {
      label: "IOR",
      pct: g
        ? statPct(gnIOR.value, g.material.ior.min, g.material.ior.max)
        : statPct(gnIOR.value, 1.5, 2.8),
      value: gnIOR.value.toFixed(2),
    },
    {
      label: "Fire",
      pct: g
        ? statPct(gnFire.value, g.material.iridescence.min, g.material.iridescence.max)
        : Math.round(gnFire.value * 100),
  ...
  <aside class="genome">
    ...
    <div class="gn-foot">
      ...
      <div class="gn-lights">
        <span
          class="gn-dot"
          :style="{ background: `hsl(${Math.round(lightHue1)}, 85%, 65%)` }"
        ></span>
        <span
          class="gn-dot"
          :style="{ background: `hsl(${Math.round(lightHue2)}, 85%, 65%)` }"
        ></span>
      </div>
    </div>
  </aside>
