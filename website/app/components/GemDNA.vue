...
import type { SeedstoneSchema, SeedstoneConfig } from 'seedstone'

const props = defineProps<{ config: SeedstoneConfig | null }>()
...
// Knob ranges come from the schema so the stat bars never drift from the
// library's real d() declarations.
const schema = ref<SeedstoneSchema | null>(null)
onMounted(async () => {
  schema.value = (await import('seedstone')).configSchema
})
...
// Flat display view of the resolved per-seed config.
const d = computed(() => {
  ...
  return {
    cut:        config.gem.cut,
    hue:        config.gem.hue,
    saturation: config.gem.saturation,
    speed:      config.gem.speed,
    tilt:       config.gem.tilt,
    ior:        config.gem.material.ior,
    fire:       config.gem.material.iridescence,
    perfection: config.gem.distortion.perfection,
    light1Hue:  config.lights.accent1Hue,
    light2Hue:  (config.lights.accent1Hue + config.lights.accent2HueOffset) % 360,
  }
})
...
const stats = computed(() => {
  if (!d.value || !schema.value) return []
  const { gem } = schema.value
  return [
    {
      label: 'Saturation',
      value: `${Math.round(d.value.saturation * 100)}%`,
      pct:   pct(d.value.saturation, gem.saturation.min, gem.saturation.max),
      color: `hsl(${d.value.hue}, 80%, 55%)`,
    },
    {
      label: 'IOR',
      value: d.value.ior.toFixed(2),
      pct:   pct(d.value.ior, gem.material.ior.min, gem.material.ior.max),
      color: '#a78bfa',
    },
    {
      label: 'Fire',
      value: `${Math.round(d.value.fire * 100)}%`,
      pct:   pct(d.value.fire, gem.material.iridescence.min, gem.material.iridescence.max),
      color: `hsl(${d.value.light1Hue}, 90%, 65%)`,
    },
    {
      label: 'Speed',
      value: `×${d.value.speed.toFixed(2)}`,
      pct:   pct(d.value.speed, gem.speed.min, gem.speed.max),
      color: '#60a5fa',
    },
    {
      label: 'Perfection',
...
<template>
  <div class="dna-panel" :class="{ loaded: !!config }">
