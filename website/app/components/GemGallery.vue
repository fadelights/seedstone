...
onMounted(async () => {
  ...
  items.value = visible.map((cut) => ({ seed: cut, label: cut, cut }));
  ...
  els.forEach((el, i) => {
    setTimeout(() => {
      ...
      gems.set(
        cut,
        new SeedstoneRenderer(item.seed, {
          container: el,
          width: s,
          height: s,
          background: BG,
          targetFPS: 24,
          config: { gem: { cut: item.cut } },
        }),
      );
  ...
  <section class="gallery-section">
    ...
    <div ref="grid" class="gallery-grid">
      <div
        ...
        @click="emit('pick', { seed: item.seed, cut: item.cut })"
      >
